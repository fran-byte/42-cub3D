...
void	load_textures(t_game *g)
{
	...
	g->map.sprites.no = mlx_xpm_file_to_image(g->mlx, g->map.paths.north,
			&tex_w[0], &tex_h[0]);
	if (!g->map.sprites.no)
		free_error(g);
	g->map.sprites.su = mlx_xpm_file_to_image(g->mlx, g->map.paths.south,
			&tex_w[1], &tex_h[1]);
	if (!g->map.sprites.su)
		free_error(g);
	g->map.sprites.we = mlx_xpm_file_to_image(g->mlx, g->map.paths.west,
			&tex_w[2], &tex_h[2]);
	if (!g->map.sprites.we)
		free_error(g);
	g->map.sprites.ea = mlx_xpm_file_to_image(g->mlx, g->map.paths.east,
			&tex_w[3], &tex_h[3]);
	if (!g->map.sprites.ea)
		free_error(g);
